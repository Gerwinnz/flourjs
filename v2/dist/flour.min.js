...
else{this.append(this.view.el);}}});};var flour=flour||{};flour.filter={defined:{}};flour.filter.add=function(filterName,filterFunction){if(!flour.filter.defined[filterName]){flour.filter.defined[filterName]=filterFunction;}};var flour=flour||{};flour.http={options:{responseType:'json'}};flour.http.prePostDataHandler=function(data,options){var contentType=options.headers&&options.headers['Content-Type']!==undefined?options.headers['Content-Type']:false;if(contentType==='application/json'){return JSON.stringify(data);}
var myFormData=new FormData()
for(var key in data){myFormData.append(key,data[key]);}
return myFormData;return JSON.stringify(data);};flour.http.postFetchResponseHandler=function(data,options){return data;};flour.http.parseURL=function(url,data){if(data!==undefined){for(var key in data){if(url.indexOf(':'+key)!==-1){var replaceString=':'+key;url=url.replace(replaceString,data[key]);delete(data[key]);}}}
...
method=method===undefined?'GET':method.toUpperCase();options.method=method;options.responseType=options.responseType!==undefined?options.responseType.toLowerCase():undefined;return function(data,extra){var parsedURL=flour.http.parseURL(url,data);extra=extra===undefined?{}:extra;console.log('options',options);if(data!==undefined){if(method==='GET'){var urlParams=[];if(parsedURL.indexOf('?')===-1){parsedURL+='?';}
for(var key in data){urlParams.push(key+'='+data[key]);}
parsedURL+=urlParams.join('&');}
else{options.body=flour.http.prePostDataHandler(data,options);}}
...
var setResponse=setValue(mValues,key,value);if(setResponse.changeList.length){console.log('state::set::'+changeEvent.type,key,value);for(var i=0,n=setResponse.changeList.length;i<n;i++){changedKey=setResponse.changeList[i];if(mKeyChangeListeners[changedKey]){changeEvent.key=changedKey;changeEvent.value=get(changedKey);callKeyChangeListeners(changedKey,changeEvent);}
...
function setValue(obj,key,value,changeList){key=(typeof key==="string")?key.split("."):key;changeList=changeList===undefined?[]:changeList;var currentKey=key.shift();var valueChanged=false;var changedKey=changeList.length>0?changeList[changeList.length-1]+'.'+currentKey:currentKey;changeList.push(changedKey);if(key.length===0){if(flour.util.isObject(value)){valueChanged=JSON.stringify(obj[currentKey])!==JSON.stringify(value);changeList=changeList.concat(getChangeListFromObject(changedKey,value));}
else{valueChanged=obj[currentKey]!==value;}
obj[currentKey]=value;return{value:value,changeList:valueChanged?changeList:false};}
else if(!obj.hasOwnProperty(currentKey)){obj[currentKey]={};}
return(setValue(obj[currentKey],key,value,changeList));}
function getChangeListFromObject(rootKey,object){var changeList=[];for(var objectKey in object){if(mKeyChangeListeners[rootKey+'.'+objectKey]!==undefined){changeList.push(rootKey+'.'+objectKey);}
if(flour.util.isObject(object[objectKey])){changeList=changeList.concat(getChangeListFromObject(rootKey+'.'+objectKey,object[objectKey],changeList));}}
return changeList;}
...
var mBlockHtml=block.html;var mValue=state.get(mKey);var mShow=false;var mTemplate=false;var cleanup=state.onChange(mKey,function(event){mValue=event.value;showContent();});var showContent=function(){var show=mValue?true:false;show=mInverse?!show:show;if(show===mShow){return;}
