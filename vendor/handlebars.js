...
//  Return calculated percentage 
//
Handlebars.registerHelper('pluralize', function(string, value) {
...
//
//  Insert spaces specified x number of times
//
Handlebars.registerHelper('spacer', function(level, string) 
{  
  if(level > 0)
  {
    var dots = '';
...
//
Handlebars.registerHelper('add_breaks', function(string)
{
	if(!string)
	{
		return;
...
//
//	Converts text or markdown to html
//
Handlebars.registerHelper('convert_markdown', function(string)
{
	if(!string)
	{
		return;
...
{
  return templates[template]({'data': data});
});
...
//
//  Return formatted date, default format MMM Do, YYYY / Jan 25th, 2014
//
Handlebars.registerHelper('date_format', function(context, block) 
{
  ...
  if(isNaN(context))
	...
	{
		var momentTime = moment.unix(context);
		return momentTime.format(f);
	}
...
//  Return formatted date, default format MMM Do, YYYY / Jan 25th, 2014
//
Handlebars.registerHelper('time_ago', function(context, block) 
{
	if(isNaN(context))
	{
...
Handlebars.registerHelper ('truncate', function (str, len) 
{
  ...
  {
    return new Handlebars.SafeString ( flour.truncate(str, len) + '...' );
  }
...
//  List users
//
Handlebars.registerHelper('list_users', function (users)
{
	var listItems = [];
  for(var i = 0, n = users.length; i < n; i ++)
  {
  	listItems.push(users[i].first_name);
  }
